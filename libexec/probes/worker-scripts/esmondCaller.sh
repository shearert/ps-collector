...
python uploader/callerEsmondUploader.py -d $debug -s $start -u $host -w $username -k $key -g $goc -t $timeout -x $summary -a $allowedEvents -c $cert -o $certkey --queue $directoryqueue --tmp $tmpdir -A $allowedMQEvents -M $maxMQmessageSize
deactivate
