...
python esmonduploader/caller.py -d $debug -s $start -u $host -p -w $username -k $key -g $goc -t $timeout -x $summary -a $allowedEvents -c $cert -o $certkey --queue $directoryqueue --tmp $tmpdir -A $allowedMQEvents -M $maxMQmessageSize
deactivate
